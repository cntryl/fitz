...
export type QueueDeadLettersQueryKey = readonly [
  "queueDeadLetters",
  string,
  string,
  string,
...
export interface QueueDeadLettersQuery extends ResourceResult<DeadLetterMessage[]> {
  key: QueueDeadLettersQueryKey;
  retryAttempts: number;
  staleTimeMs: number;
  isStale(): boolean;
}

export type { DeadLetterFilters, DeadLetterMessage, QueueResourceRef };

const fetchedAtByKey = new Map<string, number>();

function keyString(key: QueueDeadLettersQueryKey) {
  return key.join(":");
}
...
export function createQueueDeadLettersQuery(
  ...
  const result = resource(
    async ({ signal }) => {
      ...
      fetchedAtByKey.set(cacheKey, Date.now());
      return messages;
    },
    [...key],
  );
  ...
  return Object.assign(result, {
    ...
    isStale: () => Date.now() - (fetchedAtByKey.get(cacheKey) ?? 0) > QUEUE_DEAD_LETTERS_STALE_MS,
  });
}
