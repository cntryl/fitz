...
fn bench_mux_route_reuse(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        let name = format!("route_owning_record_clone_{}B", size);
        ...
        let name = format!("route_ref_zero_copy_{}B", size);
        group.bench_function(&name, |b| {
            b.iter(|| {
...
fn bench_mux_route_decode_each(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.throughput(Throughput::Elements(1));
        let name = format!("decode_then_route_owning_{}B", size);
        group.bench_function(&name, |b| {
            b.iter(|| {
...
fn bench_mux_release_and_backpressure(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_function(format!("release_after_route_ref_{}B", size), |b| {
            b.iter_batched(
                || {
        ...
        group.bench_function(format!("route_ref_channel_full_{}B", size), |b| {
            b.iter_batched(
                || {
                    let mut mux = Mux::new(1);
