...
fn bench_append_commit_single_event(c: &mut Criterion) {
    let (mut actor, mut ctx) =
        create_bench_stream_actor("bench-realm", "bench-area", "bench-single");
...
fn bench_append_commit_batches(c: &mut Criterion) {
    ...
    for &batch_size in &batch_sizes {
        let (mut actor, mut ctx) = create_bench_stream_actor(
            "bench-realm",
            "bench-area",
            &format!("bench-batch-{}", batch_size),
        );

        let payloads = create_bench_event_payloads(batch_size + 50, 256);
        let route = Route::new(format!(
            "stream://bench-realm/bench-area/bench-batch-{}/append",
            batch_size
        ));
        let family_id = RouteFamily::new(1);
...
fn bench_resource_read_batched(c: &mut Criterion) {
    let (mut actor, mut ctx) =
        create_bench_stream_actor("bench-realm", "bench-area", "bench-read-batch");
    ...
    let read_route =
        Route::new("stream://bench-realm/bench-area/bench-read-batch/read".to_string());
    let mut read_offset = 0u64;
...
fn bench_area_read_sequential(c: &mut Criterion) {
    ...
    let mut area_actor = StreamActor::new(
        family,
        realm.to_string(),
        area.to_string(),
        area_resource.to_string(),
        store.clone(),
    );
    let addr = RouteAddress::new(
        family,
...
fn bench_realm_read_sequential(c: &mut Criterion) {
    ...
    let mut realm_actor = StreamActor::new(
        family,
        realm.to_string(),
        "".to_string(),
        realm_resource.to_string(),
        store.clone(),
    );
