...
impl NoticeDomainCore {
    ...
    fn sync_admin_snapshot(&self) {
        ...
        let family_cores: Vec<_> = self
            .family_cores
            .lock()
            .values()
            .filter_map(std::sync::Weak::upgrade)
            .collect();
        for core in &family_cores {
        ...
        let mut route_metrics = HashMap::with_capacity(routes.len());
        for core in &family_cores {
            let mut route_stats = core.route_stats.lock();
            route_stats.retain(|route, stats| {
                let keep = routes.contains_key(route);
                if keep {
                    stats.prune_recent_publishes(now);
                    route_metrics.insert(
                        route.clone(),
        ...
        self.admin_read_model.replace_notice_routes(
            routes
                ...
                .map(|((route_family, route), subscribers)| {
                    let (publishes_total, publishes_per_minute) = route_metrics
                        .get(&(route_family, std::sync::Arc::clone(&route)))
                        .copied()
                        .unwrap_or((0, 0.0));
        ...
        if let Some(metrics) = &self.metrics {
            metrics.set_subscription_count(self.aggregate_subscription_count());
        }
    }
    ...
    fn aggregate_subscription_count(&self) -> usize {
        self.family_cores
            .lock()
            .values()
            .filter_map(std::sync::Weak::upgrade)
            .map(|core| core.subscription_count())
            .sum()
    }
