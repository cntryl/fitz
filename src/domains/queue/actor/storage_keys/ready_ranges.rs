...
impl QueueActor {
    ...
    pub(in crate::domains::queue::actor) fn ready_range_key(
        ...
        Self::ready_range_key_with_prefix(
            &self.persistence.recovery.ready_index_prefix,
            shard,
            start,
        )
    }
    ...
    pub(in crate::domains::queue::actor) fn delayed_index_key(
        ...
        Self::delayed_index_key_with_prefix(
            &self.persistence.recovery.delayed_index_prefix,
            visible_at_ms,
            id,
        )
    }
    ...
    pub(in crate::domains::queue::actor) fn dlq_index_key(
        ...
        let mut key = Vec::with_capacity(self.persistence.recovery.dlq_index_prefix.len() + 16);
        key.extend_from_slice(&self.persistence.recovery.dlq_index_prefix);
