        ...
        let (response, ready_notification, should_mark_admin_snapshot_dirty) = match queue_msg {
            ...
            QueueMessage::Ack {
                ...
                Ok(key) => {
                    ...
                    let actor_exec_start = Instant::now();
                    actor.process_due_work();
                    let response = actor.handle_ack_for_session(frame_ctx.session_id, id, token);
                    let notification =
                        self.record_ready_state(&notification_key, actor.admin_snapshot());
                    self.observe_histogram_us(
                        obs::METRIC_QUEUE_ACTOR_EXECUTION_LATENCY,
...
mod tests {
    ...
    fn bad_request_reason(frame: &FrameContext) -> String {
        let (code, message) =
            crate::protocol::error_codes::decode_error_body(frame.payload.as_ref())
    ...
    fn receive_response_first_message(frame: &FrameContext) -> (u64, u64) {
    ...
    fn queue_simple_error_code(frame: &FrameContext) -> u16 {
        let (code, _) = crate::protocol::error_codes::decode_error_body(frame.payload.as_ref())
            .expect("queue error envelope");
        code
    }

    fn receive_queue_frame(mailbox: &Mailbox, label: &str) -> FrameContext {
        mailbox
            .receiver()
            .try_recv()
            .expect(label)
