...
fn should_confirm_stream_family_cleanup_before_reporting_delivery() {
    ...
    .expect("create family Stream sink");
...
fn should_preserve_append_session_without_notify_given_commit_failure() {
    ...
    let durable_metrics = context.sink.durable_metrics_snapshot();
    context.sink.sync_admin_snapshot();
    let stream = context
        .admin_read_model
        .streams(None)
    ...
    assert_eq!(durable_metrics.events_total, 1);
}
