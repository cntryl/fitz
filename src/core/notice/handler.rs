...
impl NoticeDomain {
    ...
    pub async fn subscribe(&self, route_pattern: String, channel_id: u32, sender: SubSender) -> u64 {
    ...
    fn parse_operation(&self, payload: &[u8]) -> Result<NoticeOp, String> {
        ...
        if has_subscribe {
            ...
            Err("Unknown notice operation: no subscribe, unsubscribe, or body tag found".to_string())
        }
    }
        ...
        Box::pin(async move {
            ...
            match operation {
                ...
                NoticeOp::Publish => {
                    ...
                    let msg_id = self.find_tlv(&request.payload, TAG_ID)
                        .and_then(|b| std::str::from_utf8(b).ok());
                    let reply_to = self.find_tlv(&request.payload, TAG_ROUTE_REPLY)
                        .and_then(|b| std::str::from_utf8(b).ok());
                    ...
                    let mut service = self.service.lock().await;
                    let (_delivered, _failed) = service.publish(
                        &request.route_str,
                        msg_id,
                        body,
                        reply_to,
