...
async fn should_filter_schedule_operation_before_applying_limit() {
    ...
    // Act
    let response = schedule_executions_for_resource(
        &runtime,
        &path,
        &ScheduleExecutionObservationRequest {
            family: 1,
            operation: Some("target".to_string()),
            offset: 0,
            limit: 1,
        },
    );
...
#[tokio::test]
async fn should_page_schedule_operations_after_filtering_resource_scope() {
    // Arrange
    let runtime = snapshot_runtime();
    runtime.admin_read_model().replace_schedules(
        ["alpha", "bravo", "charlie"]
    ...
    assert_eq!(list.observations.len(), 1);
    assert_eq!(list.observations[0].operation, "bravo");
}
