...
// LeaseTestClient / LeaseConnector impls removed — use `fixtures::transport` helpers.

// Connector types are provided by `fixtures::transport` (TcpConnector / WsConnector).

// Lease TLV builders/parsers are provided by `fixtures::transport`.
...
async fn should_complete_acquire_renew_release_cycle_tcp() {
    ...
    fixtures::transport::should_complete_acquire_renew_release_cycle::<TcpConnector>(&server).await;
}
...
async fn should_receive_responses_within_reasonable_time_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_receive_responses_within_reasonable_time::<TcpConnector>(&server)
        .await;
}
...
async fn should_handle_concurrent_connections_with_separate_leases_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_handle_concurrent_connections_with_separate_leases::<TcpConnector>(
        &server,
    )
    .await;
}
...
async fn should_enforce_lease_contention_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_enforce_lease_contention::<TcpConnector>(&server).await;
}
...
async fn should_reject_operations_with_invalid_token_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_operations_with_invalid_token::<TcpConnector>(&server).await;
}
...
async fn should_require_connect_message_when_auth_enabled_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_require_connect_message_when_auth_enabled::<TcpConnector>(&server)
        .await;
}
...
async fn should_accept_valid_jwt_in_connect_message_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_accept_valid_jwt_in_connect_message::<TcpConnector>(&server).await;
}
...
async fn should_reject_expired_jwt_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_expired_jwt::<TcpConnector>(&server).await;
}
...
async fn should_reject_invalid_jwt_signature_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_invalid_jwt_signature::<TcpConnector>(&server).await;
}
...
async fn should_reject_jwt_for_wrong_realm_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_jwt_for_wrong_realm::<TcpConnector>(&server).await;
}
...
async fn should_create_separate_sessions_for_each_connection_with_auth_tcp() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_create_separate_sessions_for_each_connection_with_auth::<
        TcpConnector,
    >(&server)
    .await;
}
...
async fn should_reject_renew_without_acquire_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_renew_without_acquire::<TcpConnector>(&server).await;
}
...
async fn should_reject_release_after_release_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_release_after_release::<TcpConnector>(&server).await;
}
...
async fn should_support_query_operation_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_support_query_operation::<TcpConnector>(&server).await;
}
...
async fn should_enforce_fencing_token_monotonicity_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_enforce_fencing_token_monotonicity::<TcpConnector>(&server).await;
}
...
async fn should_isolate_leases_across_resources_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_isolate_leases_across_resources::<TcpConnector>(&server).await;
}
...
async fn should_timeout_on_malformed_frame_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_timeout_on_malformed_frame::<TcpConnector>(&server).await;
}
...
async fn should_handle_connection_drop_during_lease_tcp() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_handle_connection_drop_during_lease::<TcpConnector>(&server).await;
}
...
async fn should_complete_acquire_renew_release_cycle_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_complete_acquire_renew_release_cycle::<WsConnector>(&server).await;
}
...
async fn should_receive_responses_within_reasonable_time_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_receive_responses_within_reasonable_time::<WsConnector>(&server)
        .await;
}
...
async fn should_handle_concurrent_connections_with_separate_leases_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_handle_concurrent_connections_with_separate_leases::<WsConnector>(
        &server,
    )
    .await;
}
...
async fn should_enforce_lease_contention_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_enforce_lease_contention::<WsConnector>(&server).await;
}
...
async fn should_reject_operations_with_invalid_token_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_operations_with_invalid_token::<WsConnector>(&server).await;
}
...
async fn should_require_connect_message_when_auth_enabled_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_require_connect_message_when_auth_enabled::<WsConnector>(&server)
        .await;
}
...
async fn should_accept_valid_jwt_in_connect_message_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_accept_valid_jwt_in_connect_message::<WsConnector>(&server).await;
}
...
async fn should_reject_expired_jwt_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_expired_jwt::<WsConnector>(&server).await;
}
...
async fn should_reject_invalid_jwt_signature_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_invalid_jwt_signature::<WsConnector>(&server).await;
}
...
async fn should_reject_jwt_for_wrong_realm_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_jwt_for_wrong_realm::<WsConnector>(&server).await;
}
...
async fn should_create_separate_sessions_for_each_connection_with_auth_ws() {
    let server = TestServer::start_with_auth(true)
        .await
        .expect("failed to start test server");
    fixtures::transport::should_create_separate_sessions_for_each_connection_with_auth::<WsConnector>(&server).await;
}
...
async fn should_reject_renew_without_acquire_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_renew_without_acquire::<WsConnector>(&server).await;
}
...
async fn should_reject_release_after_release_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_reject_release_after_release::<WsConnector>(&server).await;
}
...
async fn should_support_query_operation_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_support_query_operation::<WsConnector>(&server).await;
}
...
async fn should_enforce_fencing_token_monotonicity_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_enforce_fencing_token_monotonicity::<WsConnector>(&server).await;
}
...
async fn should_isolate_leases_across_resources_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_isolate_leases_across_resources::<WsConnector>(&server).await;
}
...
async fn should_timeout_on_malformed_frame_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_timeout_on_malformed_frame::<WsConnector>(&server).await;
}
...
async fn should_handle_connection_drop_during_lease_ws() {
    let server = TestServer::start()
        .await
        .expect("failed to start test server");
    fixtures::transport::should_handle_connection_drop_during_lease::<WsConnector>(&server).await;
}
