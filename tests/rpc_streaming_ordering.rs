...
fn should_handle_in_order_streaming_chunks() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 0, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 1, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 2, true)), &mut ctx);
...
fn should_buffer_out_of_order_chunks() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 2, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 3, false)), &mut ctx);
...
fn should_cleanup_stream_when_final_chunk_received() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 1, true)), &mut ctx);
...
fn should_handle_multiple_concurrent_streams() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id_1, 0, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id_2, 0, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id_3, 0, false)), &mut ctx);
...
fn should_isolate_streams_by_correlation_id() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id_1, 0, true)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id_2, 0, false)), &mut ctx);
...
fn should_handle_buffer_overflow_by_disconnecting() {
    ...
    for seq in 1..=6 {
        inbox.receive(InboxMessage::Response(create_response(correlation_id, seq, false)), &mut ctx);
    }
...
fn should_handle_large_sequence_gaps() {
    ...
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 100, false)), &mut ctx);
    inbox.receive(InboxMessage::Response(create_response(correlation_id, 0, false)), &mut ctx);
