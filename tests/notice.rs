...
async fn should_deliver_notice_to_multiple_subscribers() {
    // Arrange
    let (handle, _store) = start_test_engine();
    let (tx1, _rx1) = create_sub_channel(default_sub_capacity());
    let (tx2, _rx2) = create_sub_channel(default_sub_capacity());
    let _sub1 = handle
...
#[tokio::test]
async fn should_subscribe_with_channel_id_one() {
    // Arrange
    let (handle, _store) = start_test_engine();
    let (tx, _rx) = create_sub_channel(default_sub_capacity());
    ...
    // Act
    let result = handle
        .subscribe("notice://realm/area/ch1/events".to_string(), tx, 1)
        .await;
...
#[tokio::test]
async fn should_subscribe_with_channel_id_two() {
    // Arrange
    let (handle, _store) = start_test_engine();
    let (tx, _rx) = create_sub_channel(default_sub_capacity());
    ...
    // Act
    let result = handle
        .subscribe("notice://realm/area/ch2/events".to_string(), tx, 2)
        .await;
...
async fn should_handle_subscriber_channel_full_backpressure() {
    // Arrange
    let (handle, _store) = start_test_engine();
    let (tx, _rx) = create_sub_channel(1);
    let _sub = handle
