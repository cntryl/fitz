...
// This file asserts notification fanout math: deterministic N → M delivery counts.
...
fn should_fan_out_one_notification_to_one_subscription() {
    ...
    let subscribe = SubscribeMessage::new(
        family,
        common::harness_notification::route("notify://realm/area/one/*"),
        session_id(1),
        subscriber.clone(),
    );
    ...
    let pubmsg = PublishMessage::new(
        family,
        common::harness_notification::route("notify://realm/area/one/sink"),
        Bytes::from("x"),
    );
...
fn should_fan_out_one_notification_to_many_subscriptions() {
    ...
    let pubmsg = PublishMessage::new(
        family,
        common::harness_notification::route("notify://realm/area/one/sink3"),
        Bytes::from("payload"),
    );
...
fn should_fan_out_many_notifications_to_many_subscriptions() {
    ...
    for i in 0..3 {
        ...
        let subscribe = SubscribeMessage::new(
            family,
            common::harness_notification::route("notify://realm/area/many/*"),
            session_id(i as u64 + 1),
            sub.clone(),
        );
        actor.receive(NotificationMessage::Subscribe(subscribe), &mut ctx);
    ...
    for n in 0..4 {
        let pubmsg = PublishMessage::new(
            family,
            common::harness_notification::route(&format!("notify://realm/area/many/item{}", n)),
            Bytes::from("p"),
        );
        actor.receive(NotificationMessage::Publish(pubmsg), &mut pubctx);
    }
...
fn should_produce_zero_deliveries_when_no_subscriptions_exist() {
    ...
    let pubmsg = PublishMessage::new(
        family,
        common::harness_notification::route("notify://realm/area/none/sink"),
        Bytes::from("nop"),
    );
    actor.receive(NotificationMessage::Publish(pubmsg), &mut ctx);
...
fn should_produce_exactly_n_deliveries_for_n_matching_subscriptions() {
    ...
    for i in 0..n {
        ...
        let subscribe = SubscribeMessage::new(
            family,
            common::harness_notification::route("notify://realm/area/exact/*"),
            session_id(i as u64 + 1),
            sub.clone(),
        );
        actor.receive(NotificationMessage::Subscribe(subscribe), &mut ctx);
    ...
    let pubmsg = PublishMessage::new(
        family,
        common::harness_notification::route("notify://realm/area/exact/s3"),
        Bytes::from("x"),
    );
    actor.receive(NotificationMessage::Publish(pubmsg), &mut pubctx);
